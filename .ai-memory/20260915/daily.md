...
## [04:5x] 动作: 侧边栏可收起为窄条（提交 91b741e，已推送）

- 需求：组件库侧边栏右上角加收起图标 → 收起为窄条（IDE 活动栏风格），画布腾空间
- 实现：`sidebarCollapsed` + `toggleSidebar`（持久化）；`SidebarRail`（» + 竖排文字 + 组件数徽标，整条可点展开）；收起时只渲染窄条不渲染面板；LibraryPanel 标题行加 `«` 按钮
- 实测画布 **1104 → 1394（+290px）**，展开还原
- 验证：冒烟 51 项（+2）、单测 49 例、typecheck 0 error
- 插曲：action 声明漏改（只改实现）→ TS 报错兜住

## [04:3x] 动作: 修 hover 疯狂抖动 + 组内条目分隔线（提交 7353240，已推送）

- **抖动**（用户报"鼠标停在诊断标题上疯狂上下抖、像两条警告来回切换"，截图抓不到）：上轮把说明区从固定 84px 改成 `max-height: 26vh` 导致高度可变 → 悬停时内容变多把列表推下 → 鼠标相对位置偏移 → 反复触发 onMouseLeave/onMouseEnter → **高频振荡**。修复：**固定高度 96px**（固定态 200px）+ 内部滚动
- **组内条目分隔**：R-16 的 #1/#2/#3 之间加细横线（最后一条不画线）
- **新增防抖断言**：悬停后间隔 700ms 两次采样，要求说明区高度/内容/列表 top 一致（实测 `96px / 653px` 稳定）
- 验证：冒烟 **49 项**、typecheck 0 error
- 插曲：提交信息含双引号导致 PowerShell 解析断开、commit 静默失败（第二次犯，规则已入 MEMORY）
