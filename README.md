...
| 一键校验 | 19 条规则覆盖连通性、端口合法性、引脚能力、电源与共地、协议（I2C / 单总线 / UART 交叉）、电平匹配（5V→3.3V）、保护元件与大电流负载供电 |
...
| 真实浏览器冒烟 | `npm run smoke`（前后端需已启动） | 21 项断言，0 控制台错误 |
| 主题可读性扫描 | `node apps/web/scripts/check-contrast.mjs dark\|light` | 低对比度文本 0 项 |
| 全量 | `npm test && npm run typecheck && npm run build` | 87 例通过 / 0 error |
