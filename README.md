...
| 前端单测 | `npm test -w @sim/web` | 37 例（流程集成 + 引用稳定性 + 项目管理 + 撤销重做 + 器件直连 + UI 冒烟） |
| 真实浏览器冒烟 | `npm run smoke`（前后端需已启动） | 33 项断言（含真实拖拽创建器件直连），0 控制台错误 |
| 主题可读性扫描 | `node apps/web/scripts/check-contrast.mjs dark\|light` | 低对比度文本 0 项 |
| 全量 | `npm test && npm run typecheck && npm run build` | 103 例通过 / 0 error |
...
- 组件库当前 **1 块开发板 + 14 个组件**（ESP32-DevKitC V4、DHT11、HC-SR04 超声波、SSD1306 OLED、LCD1602、LED、轻触按键、有源蜂鸣器、无源蜂鸣器、SG90 舵机、直流电机、L298N 电机驱动、USB-TTL 串口模块、电阻、3.3V 电源模块），按新增组件 SOP 持续补齐
- 连线模型：支持「组件端口 ↔ 开发板引脚」与「组件端口 ↔ 组件端口」（器件直连）；「引脚 ↔ 引脚」被禁止（那是焊接/跳线语义）。**面包板内部导通尚未实现**（需要组件内部等电位分组，属独立一轮）
