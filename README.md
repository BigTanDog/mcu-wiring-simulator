...
| 器件直连 | 支持「组件端口 ↔ 组件端口」：电机接驱动模块输出端、外设接独立电源模块；校验能识别独立电源（不再误报“缺电源来源”），并区分板载供电与独立供电 |
| 示例模板 | 「项目…」面板内置 6 个项目：4 个经典实验（温湿度显示 / 超声波测距 / 电机调速 / 串口调试）+ **综合实验项目**（26 组件 / 59 连线，性能基准的人眼可读场景）+ **面包板一拖多**（1 个 GPIO 经面包板驱动 3 路 LED）—— 均为预置正确接线，一键载入、运行即可看到结论 |
...
> 想先看看"正确接线长什么样"？打开顶栏**「项目…」→ 示例模板**，选一个载入即可（预置接线 + 一键运行）。
...
| 层级 | 命令 | 现状 |
| --- | --- | --- |
| 规则引擎单测 | `npm test -w @sim/rule-engine` | 59 例（引脚真实性 + 21 条规则正反例 + 面包板 6 例 + 器件直连 + I2C 总线豁免 + 教学说明完整性 + 可复现性） |
...
| 真实浏览器冒烟 | `npm run smoke`（前后端需已启动） | 42 项断言（含真实拖拽创建器件直连、模板载入、诊断分组折叠与展开、面包板孔位网格），0 控制台错误 |
...
- 组件库当前 **1 块开发板 + 15 个组件**（ESP32-DevKitC V4、DHT11、HC-SR04 超声波、SSD1306 OLED、LCD1602、LED、轻触按键、有源蜂鸣器、无源蜂鸣器、SG90 舵机、直流电机、L298N 电机驱动、USB-TTL 串口模块、电阻、3.3V 电源模块、**面包板**），按新增组件 SOP 持续补齐
- 连线模型：支持「组件端口 ↔ 开发板引脚」与「组件端口 ↔ 组件端口」（器件直连）；「引脚 ↔ 引脚」被禁止（那是焊接/跳线语义）。**面包板采用简化模型**（Q-T4）：每一「列」= 一个等电位端口，列内互通、列间绝缘，另有正负两条电源轨 —— 因此可以把 1 个 GPIO 引到某列后分接给多个器件（一拖多），同列上出现多个输出会由 R-23 拦下
