...
**门禁现状**：`npm test` 37 例通过、`npx tsc --noEmit` 0 error、`npm run build` 成功、`npm run smoke` 10/10 通过且 0 控制台错误。
...
| R-15 | 驱动能力超限 | warning | ⛔ 未实现 | 舵机 / 直流电机 |
| R-16 | 占用 Strapping 引脚 | warning | ✅ 已实现 | — |
| R-17 | 占用 UART0 引脚 | warning | ✅ 已实现 | — |
| R-18 | 电源短路 | error | ✅ 已实现 | — |
| R-19 | 限流/保护元件缺失 | warning | ⛔ 未实现 | LED / 感性负载 |
| R-20 | 串口 TX/RX 未交叉 | error | ⛔ 未实现 | USB-TTL 串口模块 |

**统计：14 已实现 / 6 待实现**（待实现的 6 条中，R-02/R-04 属"语义已被其他规则覆盖"，可在 Wave 2 决策是否合并而非新增）。
...
| 撤销/重做 | 未实现；建议用 zustand 时间旅行中间件（保留 20 步） | ⛔ 待实现（PRD FR-15） |
| 快捷键 | 未实现；Delete/Esc/F/Ctrl+Enter 已在 PRD §6.6 定义 | ⛔ 待实现 |
...
| GET | `/projects/:id` | 打开项目 | — | `{ project, boardDefinition, componentDefinitions[] }` |
...
| 编号 | 未完成 | 影响 | 计划 |
| --- | --- | --- | --- |
| M-01 | 前端项目 CRUD **UI** 未接后端（当前仍用 localStorage + 导入导出） | 跨设备/多项目列表不可用；后端接口已就绪（W4 已测） | W6 之后：新增"项目列表/打开/保存"面板 |
...
| M-06 | 撤销/重做、快捷键全集未实现 | FR-15 未完成 | W6 |

### 18.5 当前门禁状态（本机实测）

```
packages/rule-engine : 30 tests passed（16 条规则 + 引脚真实性 + 可复现性 + RuleConfig）
apps/web             : 15 tests passed（mock 数据源）+ 真实浏览器冒烟 12 项 passed
apps/api             : 14 tests passed（集成，含 409 冲突、数据隔离、导入导出往返、跨项目同 id 回归）
稳定性               : 后端测试连续两次运行均 14/14（已消除 SQLite 并发锁与主键冲突导致的随机失败）
