...
**统计：20 已实现 / 2 待实现**（R-02 悬空电源/地、R-04 输出-输出冲突 —— 语义已被 R-06/R-07/R-03 覆盖，建议合并而非新增；**已达到 PRD 规划的 20 条目标**）。
...
```
packages/rule-engine : 53 tests passed（20 条规则 + 器件直连与 I2C 总线豁免 + 教学说明完整性 + 引脚真实性 + 可复现性）
apps/web             : 49 tests passed（含 M-01 项目管理、FR-15 历史 12 例、器件直连 4 例、示例模板 6 例、诊断分组 6 例）+ 真实浏览器冒烟 40 项 passed（0 控制台错误）
