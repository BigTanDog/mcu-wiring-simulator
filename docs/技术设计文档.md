...
| ~~G-05~~ | ~~面包板为"端口集合 + 内部导通分组"~~ | **已实现**（简化模型，Q-T4）：每「列」即一个 `passive` 端口，列内等电位由"同端口多条连线"天然合并，无需扩展 net 语义；12 列 + 正负电源轨，自定义孔位网格渲染。实现方式见 §4.4 |
...
**面包板语义扩展（G-05，已实现）**：实际实现比原方案**更简单**——面包板的每一「列」直接建模为**一个组件端口**（`role: passive`），于是"列内等电位"不需要任何新的引擎语义：同一端口被多条连线连接时，`buildNets` 的并查集天然把它们合并成一个 net（端点 key 即 `port:<instanceId>:<portId>`）。

因此**不需要**注入合成的 `kind='bus'` 连线，也就没有"合成连线污染 `targets` 定位"的问题（原方案的 `待验证` 项一并消解）。配套决策：

- 列端口用 `passive`：方向/电压类规则（R-05/R-08）自动跳过，避免误报；
- 电源轨（`vcc` / `gnd`）用 `power` / `ground` 角色，保留电源类判定能力；
- 所有端口 `required: false`：面包板不必每个孔都接线；
- 新增 **R-23**：同一条线路上出现 ≥2 个"输出型"端口即警告（面包板同列最容易踩）；
- 同时修正 **R-19 的漏报**：原 `hasPassiveComponent` 只看"该 net 上存在 passive 端口"，面包板列端口也是 passive，会让 LED 接上面包板后不再报"缺少限流电阻"——现改为按**元件**判定（端口数 ≤2 且全部 passive，即真正的两端无源元件）。
...
| R-22 | 负载直连 GPIO（需驱动模块） | error | ✅ 已实现（读 `requirements: needs-driver`，仅拦"接 GPIO"的情形） | 直流电机 |
| R-23 | 多个输出短接在同一线路 | warning | ✅ 已实现（基于 net 判定；典型场景为面包板同列插了两个输出） | 两个超声波 ECHO 接同一列 |

**统计：21 已实现 / 2 待实现**（R-02 悬空电源/地、R-04 输出-输出冲突 —— 语义已被 R-06/R-07/R-03 覆盖，建议合并而非新增；**已达到并超过 PRD 规划的 20 条目标**）。

> 组件库同步扩展至 **1 块开发板 + 13 个组件**：DHT11、HC-SR04、SSD1306 OLED、LCD1602、LED、轻触按键、有源蜂鸣器、无源蜂鸣器、SG90 舵机、直流电机、L298N、USB-TTL、电阻；`communication`（通信模块）分类按 E-06 同步了契约枚举、分组标签与组件定义三处。
>
> **已知模型限制**：连线为「组件端口 ↔ 开发板引脚」，不支持器件直连 —— 电机接驱动模块输出端、独立电源模块供电、面包板内部导通均无法表达，需先做 Q-T2 决策（放开端口直连会牵动 net 归并、净语义与 UI 命中测试，须单独一轮）。
...
| 指标 | 目标 | 测量方法 | 现状 |
| --- | --- | --- | --- |
| 画布交互帧率 | 30 组件/50 连线 ≥ 50 FPS | 生产构建 + rAF 帧间隔采样（`bench.mjs`） | ✅ **实测 5.6 ms/帧（≈178 FPS；27 节点/58 连线），拖动中同样 5.6 ms，长任务 0** |
| 首屏可交互 | < 2 s（生产构建 + 模拟 4G） | CDP 4G 节流 + 等画布就绪 | ✅ **实测 542 ms**（TTFB 2 ms / DCL 399 ms） |
| `POST /validate` P95 | 100 组件/150 连线 < 300 ms | 基准脚本 200 次请求 | ✅ **实测 p50 15.4 ms / p95 16.6 ms / max 19 ms** |
| 端到端校验（点击运行 → 结果） | —（参考项） | 连续 5 次点击计时 | ✅ 实测中位 63 ms |
| 本地校验耗时 | < 16 ms（一帧内） | 结果面板 `durationMs`（本地结果） | ✅ 实测 1 ms（30 组件，中位） |
...
| ~~Q-T2~~ | ~~是否需要"器件并联"直连~~ **已决策（2026-09-14）：放开** | 引擎侧同步完成：R-06 识别独立电源来源、R-15 区分板载/独立供电、电机端口改为 passive（负载语义）；前端新增"允许端口↔端口"，仍禁止引脚↔引脚 | ✅ 已完成 |
| Q-T3 | 是否引入账号体系 | 决定 W3 是否增加认证模块 | 建议 v1.1 之后，用 ownerKey 过渡 |
| ~~Q-T4~~ | ~~面包板是否做插孔级~~ **已决策并实现**：保持**简化模型**（每列一个等电位端口 + 两条电源轨，共 12+2 个端口），不做插孔级（400 孔会产生数百个 Handle，画布不可用且需换渲染栈）。渲染形态由定义声明 `renderAs: 'breadboard'`，画布用孔位网格节点呈现 | 决定画布技术路线 | ✅ 已实现（15 个组件含面包板；教学效果评估仍见 T-UV6） |
| ~~Q-T5~~ | ~~诊断项是否按引脚聚合~~ **已决策并实现**：实测（综合实验项目 26 组件 / 59 连线）显示**没有同引脚重复**，真正的冗余是"同一条规则的多处命中"（R-16 ×5、R-17 ×2）→ 改为**按规则码聚合**，同规则命中折叠为一行（显示 N 处，可展开）；单处命中直接平铺 | 影响结果面板可读性 | ✅ 已实现（`diagGroups.ts`，6 例单测 + 2 项冒烟） |
...
| 编号 | 未验证项 | 验证方式 | 计划 |
| --- | --- | --- | --- |
| ~~T-UV1~~ | ~~画布帧率（30 组件/50 连线 ≥ 50 FPS）~~ **已完成**：5.6 ms/帧，拖动中 5.6 ms，长任务 0 | 生产构建 + rAF 采样（`bench.mjs`） | ✅ |
| ~~T-UV2~~ | ~~`/validate` P95（100 组件/150 连线 < 300 ms）~~ **已完成**：p95 16.6 ms（200 次） | 基准脚本 | ✅ |
| ~~T-UV3~~ | ~~首屏 < 2 s~~ **已完成**：542 ms（生产构建 + 4G 节流） | CDP 节流 + 画布就绪计时 | ✅ |
...
| **W6+** 组件补齐（M-02 第二批） | ✅ | 新增 4 组件（LCD1602 / L298N / 直流电机 / 无源蜂鸣器）+ 规则 **R-22 负载需驱动模块**（规则总数达 **20 条** = PRD 目标）；契约补 `VoltageDomain: VIN`（PRD 定义过但契约漏了）与 `needs-driver` 需求；rule-engine +6 例（含 LCD1602/OLED 地址冲突用例） |
| **W6+** 器件直连（Q-T2 决策） | ✅ | 放开「端口 ↔ 端口」：新增 `power-source` 需求 + 独立电源模块组件（共 14 个组件）、引擎加 `netHasPowerSource`（R-06 识别独立电源）、R-15 区分板载/独立供电、电机端口改 `passive`（负载语义，避免 R-06 误报）；仍禁止「引脚 ↔ 引脚」。**另修一个渲染风险**：画布加 ErrorBoundary（第三方库异常不再白屏）+ 诊断定位前校验目标是否存在（对已卸载节点 fitView 会抛异常）。rule-engine +4 例、web +4 例、冒烟 +2 项（真实拖拽创建器件直连） |
...
| ~~M-04~~ | ~~性能基准（帧率 / P95 / 首屏）未测~~ **已完成**：三项指标全部达标（帧率 5.6 ms/帧、P95 16.6 ms、首屏 542 ms），详见 `docs/性能基准.md` | NFR-01/02 ✅ 已验证 | ✅ |
| M-05 | 引脚数据未与官方手册逐项人工核对 | 真实性仅由一致性单测保障 | W2 剩余项（T-UV4） |
| ~~M-06~~ | ~~撤销/重做、快捷键全集未实现~~ **已闭环**（FR-15）：30 步历史 + 事务合并拖动 + 快捷键全集 + 顶栏按钮；**诊断列表按规则聚合**（Q-T5）：同规则多处命中折叠为一行（8 行 → 3 块） | — | ✅ 已完成 |

### 18.5 当前门禁状态（本机实测）

```
packages/rule-engine : 59 tests passed（21 条规则 + 面包板 6 例 + 器件直连与 I2C 总线豁免 + 教学说明完整性 + 引脚真实性 + 可复现性）
apps/web             : 49 tests passed（含 M-01 项目管理、FR-15 历史 12 例、器件直连 4 例、示例模板 6 例（遍历全部 7 个模板）、诊断分组 6 例）+ 真实浏览器冒烟 42 项 passed（0 控制台错误）
apps/api             : 15 tests passed（集成，含 409 冲突、数据隔离、导入导出往返、跨项目同 id、项目列表）
可读性扫描           : check-contrast.mjs 在 light / dark 两种主题下低对比度文本均为 0 项
性能基准（M-04）     : 帧率 5.6 ms/帧（≈178 FPS，27 节点/58 连线，拖动中同值，长任务 0）
                       /validate p95 16.6 ms（100 组件/150 连线，200 次）
                       首屏可交互 542 ms（生产构建 + 4G 节流）；端到端校验中位 63 ms
